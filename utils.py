# defaultdict: when key is not exist, return a default value instead of "KeyError"
...
class Board:
    ...
    scale: size of the square board (n x n), default is 20
    ...
    Then you can slice it like a NumPy array:
        ...
        e.g. read from row 3 column 4 to row 2 column 3
        $ b[3:5, 2:4]

        e.g. or just read data in row 2 column 2
        $ b[2, 2] = 1
    ...
    def _fix_evaluation(self, s, x, y, role):
        if score['BLOCKED_FOUR'] <= s < score['FOUR']:
            if s < score['BLOCKED_FOUR'] + score['THREE']:
                return score['THREE']
            elif score['BLOCKED_FOUR'] + score['THREE'] <= s < score['BLOCKED_FOUR'] * 2:
                return score['FOUR']
            else:
                return score['FOUR'] * 2
